		...
		<div id="frame-tabel">
			...
			<div class="table-responsive">
				<table class="table table-single-select datatable-pagination" id="tabel-data" width="100%">
					<thead>
						<tr>
							<th width="25%">Kode Pegawai</th>
							<th width="75%" id="pegawai_nm_table">Nama Pegawai</th>
						</tr>
					</thead>
					<tbody>
					</tbody>
		...
		<div id="frame-form">
			<form class="form-validate-jquery" id="form-data" action="#">
				...
				<div class="row">
					<div class="col-md-4">
						<div class="form-group form-group-float">
							<label class="form-group-float-label is-visible">Kode Pegawai <span class="text-danger">*</span></label>
							<input type="text" name="pegawai_cd" class="form-control text-uppercase" required="" placeholder="" aria-invalid="false">
						</div>
					</div>
					<div class="col-md-8">
						<div class="form-group form-group-float">
							<label class="form-group-float-label is-visible">Nama Pegawai <span class="text-danger">*</span></label>
							<input type="text" name="pegawai_nm" class="form-control" required="" placeholder="" aria-invalid="false">
						</div>
					</div>
				</div>
...
<script>
    ...
    $(document).ready(function(){
        ...
        tabelData = $('#tabel-data').DataTable({
            ...
            dom : 'tpi',
            columns: [
                { data: 'pegawai_cd', name: 'pegawai_cd', visible:true },
                { data: 'pegawai_nm', name: 'pegawai_nm' }
            ],
        });

        $(document).on('keyup', '#search_param',function(){ 
            tabelData.column('#pegawai_nm_table').search($(this).val()).draw();
        ...
        //--Cek kode
        $('input[name=pegawai_cd]').focusout(function(){
			var id          = $(this).val();
                ...
                $.getJSON( urlUpdate, function(data){
                    if (data['status'] == 'ok') {
                        swal({
                            ...
                            $('input[name=pegawai_cd]').val('');
                            $('input[name=pegawai_cd]').focus();
                            swal.close();
                        });
                    }
                });
        ...
        $('#reset').click(function()   {
            ...
            $('#frame-tabel').show();      
            $('#frame-form').hide(); 
            $('.card-title').text('Data Pegawai');
			$('input[name=pegawai_cd]').val(rowData['pegawai_cd']).prop('readonly',false);
        });
        ...
        $(document).on('click', '#ubah',function(){ 
            if (dataCd == null) {
				...
				$('input[name=pegawai_cd]').val(rowData['pegawai_cd']).prop('readonly',true);
                $('input[name=pegawai_nm]').val(rowData['pegawai_nm']);
    ...
    function reset(type) {
        ...
        $('#frame-tabel').show();      
        $('#frame-form').hide(); 
        $('input[name=pegawai_cd]').val('').prop('readonly',false);
        $('input[name=pegawai_nm]').val('');
        $('.card-title').text('Data Pegawai');       
    }
</script>
@endpush